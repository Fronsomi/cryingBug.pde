class Antbot 
{
  int state; // 0-дух 1-красный, 2-зеленый, 3-синий, 4-разведчик, 5-домой, 6-королеваR, 7-королеваG, 8-королеваB
  int index, prev, next; // индекс, указатель на предыдущую, указательна следующую
  float X , Y , angle, speed;  // координаты, угол, скорость
  int foodR, foodB;     // ячейки для красного, зелённого и синего ресурса
  int   disR,  disB, disH;// "расстояние" до красного, зелённого и синего ресурса и до дома
...
// новых букашек будет клепать королева
  public void newAnt(){
    ...
    ants[i].state = int(random(1,4.4)); // определим рандомно тип букашки
    ...
    ants[prev].next = i;
    prev = i;
    ants[i].speed = random(0.5, 0.8);   // скорость случайная
...
// ход обычной букашки
  public void stepA(){
    ...
    switch(cl){
      ...
      case #660000:   // обнаружен красный ресурс
        ...
        if(res < rcount){       
          ...
          if(disH>=1000 && random(1000)<1){  // если далеко от королевы, то с некоторой вероятностью
            state=7;                         // объявим себя королевой
            foodR+=food[res].reserve;        // присвоим весь ресурс
            food[res].update(); return;      // а новый ресурс появится в новом месте
          }
          if(state==1 || state==4) {         // если мы искали красный ресурс или были разведчиком
      ...
      case #000066:   // обнаружен синий ресурс
        ...
        if(res < rcount){       
          ...
          if(disH>=1000 && random(1000)<1){  // если далеко от королевы, то с некоторой вероятностью
            state=6;                         // объявим себя королевой
            foodB+=food[res].reserve;        // присвоим весь ресурс
            food[res].update(); return;      // а новый ресурс появится в новом месте
          }
          if(state==3 || state==4) {         // если мы искали синий ресурс или были разведчиком
    ...
    // пришло время озвучить один из своих счётчиков      
     ...
     what2Cry += 1; if( what2Cry == 4){  what2Cry=0; }
  }
...
// если они не достигли 1000
    ...
    X = X + 0.2*sin(angle);    // перемещение со скоростью 0,2
    Y = Y + 0.2*cos(angle);
    
    println("Queen "+index+"  жизнь="+life );  // отладка консоль
    
      // определяем, до какого ресурса королеве дальше всего
      // в этом направлении она будет смещаться
    if(disR>=disG && disR>=disB)       {  state=6; }  // до красного
     else{ if(disG>=disR && disG>=disB){  state=7; }  // до зелённого
         else                          {  state=8; }} // до синего
...
//===    отрисовка букашек  ==============
   public void render() {
     noStroke();
     switch(state){ //1-красный, 2-зеленый, 3-синий, 4-разведчик, 5-домой, 6-королева
       case 1: fill(#ff5566); ellipse(X, Y,2, 2); break;

       case 3: fill(#5555ff); ellipse(X, Y,2, 2); break;
         ...
         if (ants[cA].disB > disB+dst){             // если найденной букашке до ресурса
           ...
           if ( xz>X-dst && xz<X+dst && yz>Y-dst && yz<Y+dst){
             ...
             if( d <= dst ){                         // если она меньше дистанции слышимости
               ...
               if( ants[cA].state == 3 || ants[cA].state == 8 ){ // если найденная букашка хочет этот ресурс
                 ants[cA].setAngle(X,Y,d);                       // то повернём её к кричащему
               }                      
             }
           }
...
// заодно обовляем счетчики и разворачиваес букашек, где необходимо
      ...
      if(ant != index){                             // себя игногируем
        if (ants[ant].disH > dst){                  // если найденной букашке до Королевы
          ...
          if ( xz>X-dst && xz<X+dst && yz>Y-dst && yz<Y+dst){
            float d = dist( X,Y,xz,yz );            // посчитаем дистанцию до найденной букашки   
            if( d <= dst ){                        
              ants[ant].disH = dst;                 // то обновим у найденной букашки счётчик   
              if( ants[cA].state == 5){             // если найденная букашка идёт к Королеве
                ants[cA].setAngle(X,Y,d);           // то повернём её к себе
              }
              if( d < 20 ){                           // если найденная букашка пришла
                foodR += ants[ant].foodR; ants[ant].foodR = 0;  // заберём, что она принесла

                foodB += ants[ant].foodB; ants[ant].foodB = 0;
                ants[ant].disH = 0;                   // обнулим ей счётчик
                
                if(ants[ant].state == 5){             // если букашка и шла к Королеве   
                  ants[ant].state = int(random(1,4.4)); // то дадим ей новое задание
                  ants[ant].angle = random(TAU);        // и отпустим, куда глаза глядят
                }
              }            
            }
          }
 ...
 //===================================================
 //==    Устанавливаем угол ( на вход координаты кричащего и угол)
     public void setAngle(float x, float y, float d) {
     float an = acos((y-Y)/d);
     if (x>X) {angle = an;}
     else {angle = TAU-an;}
   }
